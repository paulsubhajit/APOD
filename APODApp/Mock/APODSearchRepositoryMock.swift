//
//  APODSearchRepositoryMock.swift
//  APODApplication
//


import Foundation
class APODSearchRepositoryMock: APODSearchRepositorySource {
    private let fetchAPODSearchRepositoryResult: RepositoryMockResult
    init(fetchAPODSearchRepositoryResult: RepositoryMockResult = .successWithValue) {
        self.fetchAPODSearchRepositoryResult = fetchAPODSearchRepositoryResult
    }
    func fetchAPODList(searchDate searchStr: String, _ completion: @escaping ((Result<APODsUsablesModel, Error>) -> Void)) {
        ...
        switch fetchAPODSearchRepositoryResult {
