//
//  APODSearchRemoteDatastoreMock.swift
//  APODApplication
//


import Foundation
class APODSearchRemoteDatastoreMock: APODSearchRemoteStoreProtocol {
    private let fetchAPODSearchRemoteResult: RemoteDatasourceMockResult
    init(fetchAPODSearchRemoteResult: RemoteDatasourceMockResult = .successWithValue) {
        self.fetchAPODSearchRemoteResult = fetchAPODSearchRemoteResult
    } 
    func getAPODList(paramater: APODSearchParamater, _ completion: @escaping ((Result<APODsUsablesModel, Error>) -> Void)) {
        ...
        switch fetchAPODSearchRemoteResult {
